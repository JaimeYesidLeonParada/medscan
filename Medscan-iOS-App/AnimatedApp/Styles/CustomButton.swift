...
struct MeddiumButton: ViewModifier {
    func body(content: Content) -> some View {
        content
            .padding(14)
            .frame(maxWidth: .infinity)
            .background(Color(hex: "6792FF"))
            .foregroundColor(.white)
            .mask(RoundedCorner(radius: 20, corners: [.topRight, .bottomLeft, .bottomRight]))
            .mask(RoundedRectangle(cornerRadius: 8))
            .shadow(color: Color(hex: "6792FF").opacity(0.5), radius: 20, x: 0, y: 10)
    }
}
...
extension View {
    ...
    func mediumButton() -> some View {
        modifier(MeddiumButton())
    }
}
